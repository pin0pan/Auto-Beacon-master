...
public class MacroFragment extends Fragment implements IAdapterListener, View.OnClickListener {
	...
	private void setInputFieldValue(Macro macro) {
		...
		if(macro.distance < Macro.MACRO_WORKS_NONE || macro.distance > Macro.MACRO_WORKS_LAST)		//나중에 LAST값 수정해야함
	...
	public void checkPermission() {
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {   // 마시멜로우 이상일 경우
			if (!Settings.canDrawOverlays(mContext)) {              // 체크
				...
				mContext.startService(new Intent(mContext, BringTopService.class));
			}
		} else {
			mContext.startService(new Intent(mContext, BringTopService.class));
		}
