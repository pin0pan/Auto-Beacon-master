...
public class MacroManager {
	...
	//MacroManager 클래스의 checkMacroWithBeacon() / checkNotFoundMacro() : 앱 시작할 때 DB에 저장된 정보 가져옴.
	//그리고 BLE 장치 스캔 후 iBeacon 장치가 검색되거나 혹은 검색되지 않았을 때 수행할 매크로가 있는지 검사한
		...
		if(beacon != null && beacon.getProximityUuid() != null) {
			for(Macro macro : mMacroList) {
				...
				if(isFound) {
					...
					//신호등 알림 테스트
					case Macro.MACRO_WORKS_NOTI_RED:
					case Macro.MACRO_WORKS_NOTI_GREEN:
					case Macro.MACRO_WORKS_NOTI_CHANGE:
					case Macro.MACRO_WORKS_NOTI_WARNING:
						macro_work.count = 1;
						macro_work.interval = 0;
						macro_work.duration = 0;
						macro_work.destination = macro.destination;
